<template>
  <div ref="rootRef" class="food-image relative w-full h-full box-border" :style="{
    '--segments-x': segments,
    '--segments-y': segments,
    ...
    <main ref="mainRef"
        ...
        <div ref="sphereRef" class="will-change-transform"
          ...
          <div v-for="(item, i) in items" :key="`${item.x},${item.y},${i}`"
            ...
            <div
              ...
              role="button" tabindex="0" @click="onTileClick"
      ...
      <div ref="viewerRef" class="absolute inset-0 z-20 pointer-events-none flex items-center justify-center"
        ...
        <div ref="frameRef" class="h-full aspect-square flex max-w-full h-auto"
...
const AUTO_ROTATE_SPEED_DEG_PER_MS = 0.002;
...
function buildItems(pool: (string | ImageItem)[], seg: number) {
  ...
  if (pool.length === 0) {
    return coords.map(c => ({ ...c, src: '', id: 0 }));
  }
  ...
  const normalizedImages = pool.map(image => {
    if (typeof image === 'string') {
      return { src: image, id: 0 };
    }
    return { src: image.src || '', id: image.id || 0 }; 
  });
...
onMounted(() => {
  ...
  resizeObserver = new ResizeObserver(entries => {
    ...
    const w = Math.max(1, cr?.width || 1);
    const h = Math.max(1, cr?.height || 1);
