<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="触发事件的用户ID，关联 sys_user.user_id" prop="userId">
        <el-input
          v-model="queryParams.userId"
          placeholder="请输入触发事件的用户ID，关联 sys_user.user_id"
          clearable
      ...
      <el-form-item label="事件时间" prop="createdAt">
    ...
    <el-table v-loading="loading" :data="EventsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="触发事件的用户ID，关联 sys_user.user_id" align="center" prop="userId" />
      <el-table-column label="事件类型：view/click/favorite/add_to_cart/purchase等" align="center" prop="eventType" />
      <el-table-column label="资源类型" align="center" prop="resourceType" />
      <el-table-column label="资源ID" align="center" prop="resourceId" />
      <el-table-column label="扩展属性，如 device/referrer" align="center" prop="properties" />
      <el-table-column label="事件时间" align="center" prop="createdAt" width="180">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:Events:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:Events:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="EventsRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="触发事件的用户ID，关联 sys_user.user_id" prop="userId">
          <el-input v-model="form.userId" placeholder="请输入触发事件的用户ID，关联 sys_user.user_id" />
        </el-form-item>
        <el-form-item label="资源ID" prop="resourceId">
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除用户行为事件（事件流）编号为"' + _ids + '"的数据项？').then(function() {
...
function handleExport() {
  proxy.download('system/Events/export', {
    ...queryParams.value
