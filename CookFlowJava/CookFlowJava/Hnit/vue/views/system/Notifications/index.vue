<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="接收用户ID，关联 sys_user.user_id" prop="userId">
        <el-input
          v-model="queryParams.userId"
          placeholder="请输入接收用户ID，关联 sys_user.user_id"
          clearable
    ...
    <el-table v-loading="loading" :data="NotificationsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="接收用户ID，关联 sys_user.user_id" align="center" prop="userId" />
      <el-table-column label="通知类型" align="center" prop="type" />
      <el-table-column label="通知负载，包含跳转信息" align="center" prop="payload" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="接收用户ID，关联 sys_user.user_id" prop="userId">
          <el-input v-model="form.userId" placeholder="请输入接收用户ID，关联 sys_user.user_id" />
        </el-form-item>
        <el-form-item label="是否已读" prop="isRead">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        userId: [
          { required: true, message: "接收用户ID，关联 sys_user.user_id不能为空", trigger: "blur" }
        ],
      }
    };
  },
